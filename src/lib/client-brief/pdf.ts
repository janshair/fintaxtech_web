...
import { createBriefRules } from './rules';
import type { BriefDefinition } from './definitions';
import type { BriefState } from './types';

export async function createClientBriefPDF(
  definition: BriefDefinition,
  state: BriefState,
  completed = new Date(),
): Promise<Blob> {
  const { copy: c } = definition;
  const { briefSummary, invalidSection } = createBriefRules(definition.sections);
  if (invalidSection(state) !== -1) throw new Error('Incomplete client brief');
  ...
  y = 38;
  heading(c.title);
  if (definition.subtitleAnswer) text(String(state.answers[definition.subtitleAnswer]));
  text(
  ...
  if (definition.includeWorkflowInPDF) text(c.workflow, t.type.small);
